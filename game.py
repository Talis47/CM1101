...
def remove_punct(text):
    ...
    #if character is not defined as punctuation then its added back into the string.
...
def normalise_input(user_input):
    ...
    ##removes spaces,makes it lower case, and punctuation
...
def display_room(room):
    ...
    print(blank_line + name + blank_line + blank_line + description + blank_line)

    ##prints blank lines - calls name dictionary and makes it uppercase, description calls on desription dictionary, prints all 3 
...
def exit_leads_to(exits, direction):
    ...
    ##goes dict rooms, goes dict exits and gets names and directions 
...
def print_menu_line(direction, leads_to):
    ...
    final_string = "Go " + direction.upper() + " to " + leads_to + "."
    print(final_string)
...
def menu(exits):
    ...
    while True:
        ...
        # Read player's input
        userinput = input("Please select an exit : ")

        # Normalise the input
        normalised_input = normalise_input(userinput)

        # Check if the input makes sense (is valid exit)
         # If so, return the player's choice

        if is_valid_exit(exits, normalised_input) == True:
